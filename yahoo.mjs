...
// Optional: &reduce=monthEnd  keeps only the last price of each month (much smaller
//           response; building month-ends from DAILY prices avoids gaps in Yahoo's monthly bars)
//
...
export async function GET(request) {
  ...
  const reduce = url.searchParams.get("reduce");
  const results = await Promise.all(symbols.map(s => fetchChart(s, range, interval)));
  const out = {};
  results.forEach(r => {
    if (reduce === "monthEnd" && r.points) {
      const byMonth = new Map();
      r.points.forEach(p => byMonth.set(p[0].slice(0, 7), p)); // oldest first, so the last one wins
      out[r.symbol] = { ...r, points: [...byMonth.values()] };
    } else {
      out[r.symbol] = r;
    }
  });
  ...
  return json({ fetchedAt: new Date().toISOString(), range, interval, reduce: reduce || null, data: out }, 200, anyFailed ? 300 : 21600);
}
