...
// Google retires older models for new users. When a model is retired, Google's error
// message names its replacement ("...use models/gemini-x.y-flash..."), and the code
// automatically tries that model next, so the site keeps working without code changes.
...
function suggestedModel(message) {
  // the replacement is the model named after "use"; otherwise the last model mentioned
  const text = message || "";
  const after = /use\s+models\/(gemini-[a-z0-9.-]*[a-z0-9])/i.exec(text);
  if (after) return after[1];
  const all = [...text.matchAll(/models\/(gemini-[a-z0-9.-]*[a-z0-9])/gi)].map(x => x[1]);
  ...
  try {
    const res = await fetch(`${API_BASE}/${encodeURIComponent(model)}:generateContent`, {
      ...
      body: JSON.stringify({
        ...
        // Newer Gemini models "think" before answering, and that thinking uses up the same
        // token allowance, so the allowance is generous and truncated answers are rejected below.
    ...
    if (!text) throw new Error(`Gemini returned no text (${candidate.finishReason || "no reason given"})`);
    if (candidate.finishReason && candidate.finishReason !== "STOP") {
      throw new Error(`Gemini's answer was cut off (${candidate.finishReason})`);
    }
    return text;
  } finally {
    clearTimeout(timer);
...
export async function GET(request) {
  ...
  report.results = [];
  const queue = models.slice(), tried = new Set();
  while (queue.length && tried.size < MAX_MODEL_ATTEMPTS) {
    ...
    try {
      const res = await fetch(`${API_BASE}/${encodeURIComponent(model)}:generateContent`, {
        ...
        headers: { "Content-Type": "application/json", "x-goog-api-key": key },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "Reply with the single word OK." }] }], generationConfig: { maxOutputTokens: 1024 } })
      });
      const data = await res.json().catch(() => ({}));
      const cand = data?.candidates?.[0] || {};
      const text = (cand.content?.parts || []).filter(p => !p.thought).map(p => p.text || "").join("").trim();
      const error = data?.error?.message || null;
      report.results.push({ model, status: res.status, ok: res.ok && Boolean(text), reply: text.slice(0, 40) || null, finishReason: cand.finishReason || null, error });
...
export async function POST(request) {
  ...
  const queue = [process.env.GEMINI_MODEL, ...FALLBACK_MODELS].filter((m, i, a) => m && a.indexOf(m) === i);
  const tried = new Set();
  ...
  while (queue.length && tried.size < MAX_MODEL_ATTEMPTS) {
    ...
    try {
      ...
      if (bad.length) { errors.push(`${model}: used numbers not in the plan (${bad.slice(0, 4).join(", ")})`); continue; }
      const last = paragraphs[paragraphs.length - 1] || "";
      if (paragraphs.length < 3 || !/[.!?]["')]?$/.test(last)) { errors.push(`${model}: answer incomplete`); continue; }
      ...
      if (next && !tried.has(next)) { queue.unshift(next); errors.push(`${model}: retired, trying ${next}`); }
      else errors.push(`${model}: ${msg.slice(0, 90)}`);
      if (err.status === 401 || err.status === 403) break;
    }
  }
  return respond({ error: errors.join(" | ") || "No model answered", fallback: true }, 502);
}
